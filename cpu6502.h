...
public:
	...
	uint8_t stkp = 0x00; //Stack poiinter
...
public:
	...
	void clock();
	void reset();
	void irq();//interrupt request signal-can be disabled
	void nmi(); //non maskable interrupt request signal-cannot be disabled
	...
	uint16_t addr_rel = 0x00;   // Represents absolute address relative to a branch
	uint8_t  opcode = 0x00;   // variable to store opcode
	uint8_t  cycles = 0;	   // remaining cycles for the instruction
...
private:
	...
	//Convenience functions to access status in public
