...
olc6502::olc6502()
{
	...
	// 16x16 table looks tremendously scary lol
	using a = olc6502;
	lookup =
	{
...
void olc6502::write(uint16_t a, uint16_t d)
{
...
uint8_t olc6502::ABX() // Absolute with X Offset
{
	uint16_t lo = read(pc);
	pc++;
	uint16_t hi = read(pc);
...
uint8_t olc6502::BCS()
{
	if (GetFlag(C) == 1)
	{
		cycles++; //in notes so as to why this hould be done
		addr_abs = pc + addr_rel;

		if ((addr_abs & 0xFF00) != (pc & 0xFF00)) //crossing page boundary
			cycles++;
...
{
	fetch();
	temp = (uint16_t)a + (uint16_t)fetched + (uint16_t)GetFlag(C); //done in 16 bit to capture ay carry bit (oveflow v bit)
	SetFlag(C, temp > 255);
	SetFlag(Z, (temp & 0x00FF) == 0);
	SetFlag(V, (~((uint16_t)a ^ (uint16_t)fetched) & ((uint16_t)a ^ (uint16_t)temp)) & 0x0080);
	SetFlag(N, temp & 0x80);
	a = temp & 0x00FF;
	return 1; // to add additional clock cycle
}
